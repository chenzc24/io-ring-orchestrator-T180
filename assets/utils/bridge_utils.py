...
def _load_vb_env() -> None:
    ...
    Lookup priority (first match wins):
      ...
      2. Skill-root .env (next to this bridge_utils.py) — per-skill bridge config.
         Allows different skills (T28, T180) to target different Virtuoso sessions
         on different hosts/ports.
      3. Nearest .env containing VB_REMOTE_HOST / VB_LOCAL_PORT, searching cwd
         and its parents  — project-level config
      4. ~/.virtuoso-bridge/.env  — user-level fallback (virtuoso-bridge init)
    """
    # 1. Explicit override
    ...
    # 2. Skill-root .env — per-skill bridge config (e.g. different Virtuoso sessions)
    skill_root = Path(__file__).resolve().parent.parent.parent
    skill_env = skill_root / ".env"
    if skill_env.is_file():
        try:
            text = skill_env.read_text(encoding="utf-8", errors="replace")
        except OSError:
            text = ""
        if "VB_REMOTE_HOST" in text or "VB_LOCAL_PORT" in text:
            load_dotenv(dotenv_path=str(skill_env), override=True)
            return

    # 3. Project-level: walk up from cwd looking for a .env with VB vars
...
def check_bridge_installed() -> tuple[bool, str]:
    ...
    try:
        ...
        return False, (
            ...
            "Install it into the T180 skill's .venv:\n"
...
def _download_via_cat(ssh, remote_path: str, local_path: Path, timeout: int = 60) -> Optional[str]:
    """Download a binary file using xxd hex dump over ssh.run_command().

    Uses 'xxd -p' to hex-encode the file on the remote, then decodes locally.
    This avoids SCP/tar-pipe failures caused by remote shell output that
    corrupts the binary stream.
    Returns None on success, or an error string.
    """
    import binascii
    try:
        r = ssh.run_command(f"xxd -p {shlex.quote(remote_path)} | tr -d '\\n'", timeout=timeout)
        rc = getattr(r, "returncode", 1)
        if rc != 0:
            return f"xxd failed (rc={rc}): {(getattr(r,'stderr','') or '').strip()}"
        hex_data = (getattr(r, "stdout", "") or "").strip()
        if not hex_data:
...
def _get_calibre_remote_base() -> str:
    ...
    return f"/tmp/vb_t180_calibre{suffix}"
...
def _download_calibre_output(ssh, remote_root: str, local_root: str, timeout: int = 180,
    ...
    """Pull `drc/` and `lvs/` subtrees from `remote_root` back to `local_root`.

    Returns None on success, or an error string. Missing remote subdirs are
    silently skipped.
    ...
    Download strategy: tar-pipe (ssh.download_file recursive) as primary,
    SCP as fallback for individual files.
    """
    import time as _time
    ...
    for attempt in range(1, max_retries + 2):  # 1 initial + max_retries
        ...
        for remote_dir in present:
            ...
            # Phase 1: tar-pipe download for the entire directory
            try:
                res = ssh.download_file(remote_dir, local_target, timeout=timeout, recursive=True)
                rc = getattr(res, "returncode", 1)
                if rc == 0:
                    # Check if we actually got content
                    if any(local_target.rglob("*")):
                        continue
            except Exception as e:
                print(f"[download] tar-pipe failed for {name}: {e}")

            # Phase 2: xxd hex dump fallback — download individual files
            try:
                find_result = ssh.run_command(
                    f"find {shlex.quote(remote_dir)} -type f",
                    timeout=60,
                )
                for line in (getattr(find_result, "stdout", "") or "").splitlines():
                    remote_file = line.strip()
                    if not remote_file:
                        continue
                    rel = remote_file[len(remote_dir):].lstrip("/")
                    local_file = local_target / rel
                    local_file.parent.mkdir(parents=True, exist_ok=True)
                    try:
                        err = _download_via_cat(ssh, remote_file, local_file, timeout=timeout)
                        if err:
                            print(f"[download] xxd failed for {remote_file}: {err}")
                    except Exception as e:
                        print(f"[download] xxd error for {remote_file}: {e}")
            except Exception as e:
                print(f"[download] find/SCP fallback failed for {name}: {e}")
...
def execute_csh_script(script_path: str, *args, timeout: int = 600) -> str:
    ...
    # Read CDS_LIB_PATH_180 directly from the project .env file (raw dotenv parse)
    ...
    cds_lib = _read_env_raw("CDS_LIB_PATH_180")
    ...
    if cds_lib:
        env_parts.append(f"CDS_LIB_PATH_180={shlex.quote(cds_lib)}")
